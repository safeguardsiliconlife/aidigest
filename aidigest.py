...
async def read_ignore_file(
    input_dir: str, filename: str = ".aidigestignore"
) -> List[str]:
...
class IgnoreFilter:
    ...
    def ignores(self, path: str) -> bool:
        ...
        for pattern in self.patterns:
            ...
            if relative_path == pattern or relative_path.startswith(
            ...
            if os.path.isfile(path):
                ...
                for root, dirs, files in os.walk(path):
                    ...
                    dirs[:] = [
                        d
                        for d in dirs
                        if not exclude_filter.ignores(os.path.join(root, d))
                    ]
                    # Process files
                    for file in files:
...
async def aggregate_files(
    ...
    user_ignore_patterns = await read_ignore_file(
        os.getcwd()
        ...
        if await is_text_file(file):
            try:
                async with aiofiles.open(file, "r", encoding="utf-8") as f:
                    ...
                    if remove_whitespace_flag and not any(
                        file.endswith(ext) for ext in WHITESPACE_DEPENDENT_EXTENSIONS
                    ):
    ...
    if use_default_ignores:
        print(
            format_log(
                f"Files ignored by default patterns: {default_ignored_count}", "🚫"
            )
        )
    if custom_ignored_count > 0:
        print(
            format_log(
                f"Files ignored by .aidigestignore and exclude patterns: {custom_ignored_count}",
                "🚫",
            )
        )
    print(
        format_log(f"Binary and SVG files included: {binary_and_svg_file_count}", "📦")
    )

    if file_size_in_bytes > MAX_FILE_SIZE:
        print(
            format_log(
                f"Warning: Output file size ({file_size_in_bytes / 1024 / 1024:.2f} MB) exceeds 10 MB.",
                "⚠️",
            )
        )
    ...
    print(
        format_log(f"Info file written to {os.path.join(output_dir, 'info.txt')}", "📄")
    )
...
def list_recent_outputs(base_dir: str):
    ...
    recent_folders = sorted(
        [
            f
            for f in os.listdir(aidigest_dir)
            if os.path.isdir(os.path.join(aidigest_dir, f))
        ],
        reverse=True,
    )[:5]
    ...
    if os.path.isfile(latest_aidigest):
        ...
        print(
            format_log(f"LATEST_AIDIGEST set to: {os.environ['LATEST_AIDIGEST']}", "🔗")
        )
    else:
...
def open_latest_aidigest():
    if "LATEST_AIDIGEST" in os.environ and os.path.isfile(
        os.environ["LATEST_AIDIGEST"]
    ):
        subprocess.run(["vim", os.environ["LATEST_AIDIGEST"]])
    else:
        print(
            format_log(
                "LATEST_AIDIGEST is not set or the file does not exist. Run aidigest or use -l option first.",
                "⚠️",
            )
        )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["."],
        help="Input files or directories (glob patterns supported)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--no-default-ignores",
        action="store_false",
        dest="default_ignores",
        help="Disable default ignore patterns",
    )
    parser.add_argument(
        "--whitespace-removal", action="store_true", help="Enable whitespace removal"
    )
    parser.add_argument(
        "--show-output-files",
        action="store_true",
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="List recent aidigest outputs"
    )
    parser.add_argument(
        "-v", "--view", action="store_true", help="Open the latest aidigest file in Vim"
    )
    ...
    if args.list:
        ...
        asyncio.run(
            aggregate_files(
                ...
                args.exclude,
                args.output,
                args.default_ignores,
                args.whitespace_removal,
                args.show_output_files,
                command,
            )
        )
